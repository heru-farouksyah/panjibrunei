...
async function open(viewport) {
  const ctx = await browser.newContext({ viewport, screen: viewport, hasTouch: true, isMobile: true, deviceScaleFactor: 2 });
...
// ---------------- PHONE ----------------
{
  ...
  check('top-left pause button present + ≥44px', await page.evaluate(() => {
    const r = document.getElementById('pause-btn')?.getBoundingClientRect();
    return r && r.left < 70 && r.top < 110 && Math.min(r.width, r.height) >= 44;
  }));
  ...
  // pause/resume toggle (top-left)
  await page.click('#pause-btn'); await page.waitForTimeout(250);
  check('pause button → paused state', await page.evaluate(() => document.getElementById('pause-btn').classList.contains('paused')));
  await page.click('#pause-btn'); await page.waitForTimeout(250);
  check('pause button → resumed', await page.evaluate(() => !document.getElementById('pause-btn').classList.contains('paused')));
...
// ---------------- TABLET ----------------
{
  ...
  check('tablet pause button present', await vis(page, '#pause-btn'));
