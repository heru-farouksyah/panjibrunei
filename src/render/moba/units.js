...
export function buildBahtera(team = 0) {
  ...
  const roof = new THREE.Mesh(new THREE.BoxGeometry(1.55, 0.2, 1.35), pbr(0xd64545, 0.5, 0.2, 0xd64545, 0.32)); roof.position.set(-0.7, 1.55, 0); g.add(roof);   // sig: crimson
  for (const [mx, sh] of [[0.7, 2.3], [-1.45, 1.9]]) {
    ...
    const trim = new THREE.Mesh(new THREE.PlaneGeometry(1.55, 0.22), new THREE.MeshStandardMaterial({ color: 0xd64545, roughness: 0.7, side: THREE.DoubleSide, emissive: 0xd64545, emissiveIntensity: 0.3 })); trim.position.set(mx - 0.04, 1.25 + sh - 0.11, 0); trim.rotation.y = Math.PI / 2; g.add(trim);
  }
...
export function buildMeriam(team = 0) {
  ...
  const base = new THREE.Mesh(new THREE.CylinderGeometry(0.55, 0.7, 0.5, 10), pbr(0xe8852e, 0.5, 0.3, 0xe8852e, 0.3)); base.position.set(-0.2, 1.35, 0); g.add(base);   // sig: orange
...
export function buildHammerhead(team = 0) {
  ...
  const cap = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.18, 0.95), new THREE.MeshStandardMaterial({ color: 0x18c8a0, roughness: 0.4, emissive: 0x18c8a0, emissiveIntensity: 0.45 })); cap.position.set(-0.3, 1.3, 0); g.add(cap);   // sig: aqua cabin cap
  const stripe = new THREE.Mesh(new THREE.BoxGeometry(3.85, 0.16, 0.06), new THREE.MeshStandardMaterial({ color: 0x18c8a0, roughness: 0.5, emissive: 0x18c8a0, emissiveIntensity: 0.5 })); stripe.position.set(0, 0.72, 0.61); g.add(stripe);   // sig: aqua
...
export function buildNakhoda(team = 0) {
  ...
  const roof1 = new THREE.Mesh(new THREE.ConeGeometry(1.7, 0.6, 4), pbr(0xf0c419, 0.5, 0.25, 0xf0c419, 0.35)); roof1.rotation.y = Math.PI / 4; roof1.position.set(0, 2.0, 0); g.add(roof1);   // sig: gold
...
export function buildTempest(team = 0) {
  ...
  for (const y of [1.4, 2.0, 2.6]) { const orb = new THREE.Mesh(new THREE.SphereGeometry(0.2, 10, 8), new THREE.MeshStandardMaterial({ color: 0xc9b0ff, emissive: 0xa06bff, emissiveIntensity: 1.1 })); orb.position.set(-0.4, y, 0); g.add(orb); }   // sig: violet
  const fin = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.5, 1.0), new THREE.MeshStandardMaterial({ color: 0xa06bff, roughness: 0.4, emissive: 0xa06bff, emissiveIntensity: 0.45 })); fin.position.set(-1.6, 0.9, 0); g.add(fin);
  g.traverse((m) => { if (m.isMesh) { m.castShadow = true; m.receiveShadow = true; } });
...
export function buildSentinel(team = 0) {
  ...
  const boss = new THREE.Mesh(new THREE.CylinderGeometry(0.4, 0.4, 0.3, 12), pbr(0x27c46a, 0.4, 0.4, 0x27c46a, 0.35)); boss.rotation.z = Math.PI / 2; boss.position.set(2.18, 0.95, 0); g.add(boss);   // sig: green
  const band = new THREE.Mesh(new THREE.BoxGeometry(0.16, 1.5, 2.0), new THREE.MeshStandardMaterial({ color: 0x27c46a, roughness: 0.45, emissive: 0x27c46a, emissiveIntensity: 0.35 })); band.position.set(1.72, 0.9, 0); g.add(band);   // sig: green shield band
